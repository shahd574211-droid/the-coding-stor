...
export default async function CheckoutPage() {
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-8 text-start" dir={locale === "ar" ? "rtl" : "ltr"}>
      <h1 className="text-2xl md:text-3xl font-bold text-primary-on-dark">{t("checkout.title")}</h1>
      <Card className="glass-card-on-dark border-white/20">
        <CardHeader className="space-y-2">
          <h2 className="text-xl font-semibold text-primary-on-dark">{t("checkout.signInRequired")}</h2>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <Link href="/login?redirect=/checkout">
            <Button className="rounded-xl">{t("checkout.login")}</Button>
          </Link>
        </CardContent>
      </Card>
    </div>
